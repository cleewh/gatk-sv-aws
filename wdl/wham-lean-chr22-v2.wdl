...
task RunWham {
    ...
    command <<<
        ...
        # Extract chr22 as BAM (~2 GB, fits in DYNAMIC /tmp)
        echo "$(date) Extracting chr22 BAM..."
        /opt/samtools/bin/samtools view -b -h -@ 4 -T ~{ref_fasta} \
        ...
        # Run whamg-lean with sliding-window mode on chr22
        echo "$(date) Running whamg-lean --lean on chr22..."
        whamg-lean --lean \
            -x 4 \
            -c chr22 \
            -a ~{ref_fasta} \
